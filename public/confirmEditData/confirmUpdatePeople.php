...
$confirmUpdatePeoplePage = new WebPage("Page de confirmation : People");
...
$r = MyPdo::getInstance()->prepare(<<<SQL
    UPDATE people
    SET name = ?, placeofbirth = ?, birthday = STR_TO_DATE(?,'YYYY-MM-DD'), deathday = STR_TO_DATE(?,'YYYY-MM-DD'), biography = ?
    WHERE id = ?
SQL);
$r -> bindValue(1, $name);
$r -> bindValue(2, $placeofbirth);
$r -> bindValue(3, $date_Birth);
$r -> bindValue(4, $date_Death);
$r -> bindValue(5, $bio);
$r -> bindValue(6, $id);
$r -> execute();

$confirmUpdatePeoplePage -> appendContent(<<<HTML
    <header class="header">
        <h1>Modifier - {$name}</h1>
    </header>
        <h2>Réussite ! </h2>
        <p>L'acteur/ice a bien été modifié</p>
        <a class="menu_return_button" href="../people.php?peopleId={$id}">
            ...
            <p>Retour à l'acteur/ice</p>
        </a>
    <footer class="footer">
    </footer>
HTML);

echo $confirmUpdatePeoplePage ->toHTML();
